...
let usuarios = [];
const admin = ["laura12"];
let productosC = ["Hamburguesa", "Pizza", "Hotdog"];
let productosB = ["CocaCola", "Pepsi", "Sprite"];
let ventas = [];
let repetir = "si";
function agregarProducto(lista, mensaje) {
...
function eliminar(lista, producto) {
...
while (repetir === "si") {
    const tipUser = Number(prompt("¿Qué tipo de usuario eres? 👤\n1. Administrador\n2. Cliente"));
    switch (tipUser) {
        case 1:
            const nameAdA = prompt("Digite contraseña de administrador");
            if (nameAdA) {
                const nameAd = nameAdA.toLowerCase();
                if (admin.includes(nameAd)) {
                    alert("Acceso permitido");
                    const val = Number(prompt("¿Qué desea hacer?\n 1. Agregar nuevos productos\n 2. Eliminar productos existentes"));
                    switch (val) {
                        case 1:
                            const op = Number(prompt("¿En qué sección desea agregar el producto?\n1. Comidas\n2. Bebidas"));
                            switch (op) {
                                case 1:
                            ...
                            break;
                        case 2:
                            const eli = prompt("¿Qué producto desea eliminar?\n" +
                                productosC.join("\n") + "\n" + productosB.join("\n" + "\n"));
                            if (eli) {
        ...
        case 2:
            ...
            if (usuario) {
                ...
                let pedido;
                do {
                    const opcion = Number(prompt("¿Qué quieres pedir? (Digite el número de la opción)\n1. Comida 🍔\n2. Bebidas 🥤"));
                    switch (opcion) {
                        case 1:
                            const comida = (_a = prompt("¿Qué desea comer? (Escriba la comida separada por comas)\n" + productosC.join("\n"))) === null || _a === void 0 ? void 0 : _a.toLowerCase().trim();
                            if (comida) {
                            ...
                            break;
                        case 2:
                            const bebidas = (_b = prompt("¿Qué desea tomar? (Escriba las bebidas separadas por comas)\n" + productosB.join("\n"))) === null || _b === void 0 ? void 0 : _b.toLowerCase().trim();
                            if (bebidas) {
